...
export function ClientTasks({ clientId }: { clientId: string }) {
  ...
  const fetchTasks = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/clients/tasks?clientId=${clientId}`);
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/clients/tasks`, {
        ...
        body: JSON.stringify({ clientId, text: newTask.trim() }),
      });
  ...
  const handleToggle = async (task: Task) => {
    ...
    try {
      const res = await fetch(`/api/clients/tasks`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId, taskId: task._id, completed: !task.completed }),
      });
